    ...
    , sgd { s }
{
    ...
    for (vector<CommonSEGD::ChannelSetHeader>& sets : sgd.common().channel_sets) {
        uint16_t ch_sets_per_scan_type_num = sgd.common().general_header.channel_sets_per_scan_type == 1665 ? sgd.common().general_header2.ext_ch_sets_per_scan_type : sgd.common().general_header.channel_sets_per_scan_type;
        if (sets.size() != ch_sets_per_scan_type_num)
            throw Exception(__FILE__, __LINE__, "Size of vector with channel sets not equal to number of channel sets in general header.");
    ...
    uint32_t extended_blocks = sgd.common().general_header.extended_hdr_blocks == 165 ? sgd.common().general_header2.extended_hdr_blocks : sgd.common().general_header.extended_hdr_blocks;
    uint32_t external_blocks = sgd.common().general_header.external_hdr_blocks == 165 ? sgd.common().general_header2.external_hdr_blocks : sgd.common().general_header.external_hdr_blocks;
...
void OSEGDRev2_1::write_trace(Trace& t)
{
    if (pimpl->chans_written == chans_in_record())
        throw Exception(__FILE__, __LINE__, "you tring write more traces than specified in channel set headers");
...
OSEGDRev2_1::OSEGDRev2_1(string file_name, CommonSEGD::GeneralHeader gh,
    ...
    : OSEGD { move(file_name), move(gh), move(gh2), {}, move(ch_sets), move(add_ghs) }
