...
string segy::ascii_to_ebcdic(string &ascii)
{
    ...
    for (auto from = ascii.begin(), to = result.begin(), end = ascii.end();
         from != end; ++from)
        *to = static_cast<char>(a2e[static_cast<uint8_t>(*from)]);
...
void segy::set_bin_hdr(binary_header const &b_h) {pimpl->d_bin_hdr = b_h;}
void segy::set_bin_hdr(binary_header &&b_h) {pimpl->d_bin_hdr = b_h;}
string const &segy::text_hdr() {return pimpl->d_text_header;}
void segy::set_text_hdr(string const &t_h) {pimpl->d_text_header = t_h;}
