	...
	if (sgy.p_bin_hdr().samp_per_tr) {
		...
		write_trace = [this](Trace &tr) {
			...
			if (sgy.p_file().tellg() == first_trace_pos) {
				...
				Trace::Header::Value v = *tr.header().get("SAMP_INT");
				double samp_int;
				if (holds_alternative<int16_t>(v))
				...
				sgy.p_file().seekg(CommonSegy::TEXT_HEADER_SIZE, ios_base::beg);
				sgy.write_bin_header();
			}
			sgy.write_trace_header(tr.header());
			sgy.write_trace_samples(tr, samp_num);
		};
	}
}
