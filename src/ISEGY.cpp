...
namespace sedaman
{
    ...
    public:
        Impl(string name, vector<map<uint32_t, pair<string, CommonSEGY::TrHdrValueType>>> tr_hdr_over,
            ...
            : common(move(name), fstream::in | fstream::binary, {}, move(add_hdr_map)), tr_hdr_io_map(tr_hdr_over)
        {
        ...
        Impl(string name, CommonSEGY::BinaryHeader bh,
             ...
             vector<pair<string, map<uint32_t, pair<string, CommonSEGY::TrHdrValueType>>>> add_hdr_map)
            : common(move(name), fstream::in | fstream::binary, move(bh), move(add_hdr_map)), tr_hdr_io_map(tr_hdr_over)
        {
            initialization(true);
        }
