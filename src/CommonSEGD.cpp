...
CommonSEGD::ChannelSetHeader::ChannelSetHeader(int stn, uint16_t chsn, uint8_t cht,
    ...
    , channel_type { cht }
    , channel_set_start_time { chsstm }
    , channel_set_end_time { chsetm }
    , descale_multiplier { dm }
    , number_of_channels { noch }
    , subscans_per_ch_set { spchs }
    , channel_gain { chg }
    , alias_filter_freq { aff }
    , alias_filter_slope { afs }
    , low_cut_filter_freq { lcff }
    , low_cut_filter_slope { lcfs }
    , first_notch_filter { fnf }
    , second_notch_filter { snf }
    , third_notch_filter { tnf }
    , ext_ch_set_num { echsn }
    , ext_hdr_flag { ehf }
    , trc_hdr_ext { the }
    , vert_stack { vs }
    , streamer_no { sn }
