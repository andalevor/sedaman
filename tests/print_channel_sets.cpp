    ...
    try {
        ...
        for (auto& scan : ch_sets) {
            ...
            for (auto& hdr : scan) {
                ...
                p(hdr.channel_set_number, name_as_string(name::CHANNEL_SET_NUMBER));
                p(hdr.channel_type, name_as_string(name::CHANNEL_TYPE));
                p(hdr.channel_set_start_time, name_as_string(name::CHANNEL_SET_START_TIME));
                p(hdr.channel_set_end_time, name_as_string(name::CHANNEL_SET_END_TIME));
                p(hdr.descale_multiplier, name_as_string(name::DESCALE_MULTIPLIER));
                p(hdr.number_of_channels, name_as_string(name::NUMBER_OF_CHANNELS));
                p(hdr.subscans_per_ch_set, name_as_string(name::SUBSCANS_PER_CH_SET));
                p(hdr.channel_gain, name_as_string(name::CHANNEL_GAIN));
                p(hdr.alias_filter_freq, name_as_string(name::ALIAS_FILTER_FREQ));
                p(hdr.alias_filter_slope, name_as_string(name::ALIAS_FILTER_SLOPE));
                p(hdr.low_cut_filter_freq, name_as_string(name::LOW_CUT_FILTER_FREQ));
                p(hdr.low_cut_filter_slope, name_as_string(name::LOW_CUT_FILTER_SLOPE));
                p(hdr.first_notch_filter, name_as_string(name::FIRST_NOTCH_FILTER));
                p(hdr.second_notch_filter, name_as_string(name::SECOND_NOTCH_FILTER));
                p(hdr.third_notch_filter, name_as_string(name::THIRD_NOTCH_FILTER));
                ...
                if (segd.general_header().add_gen_hdr_blocks && segd.general_header2()->segd_rev_major > 2) {
                    ...
                    p(*hdr.physical_unit(), name_as_string(name::PHYSICAL_UNIT));
                    p(*hdr.filter_delay(), name_as_string(name::FILTER_DELAY));
                    p(hdr.description()->data(), name_as_string(name::DESCRIPTION));
                }
                std::cout << '\n';
            }
        }
