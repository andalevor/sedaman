...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        auto name_as_string =
		   	sedaman::CommonSEGY::BinaryHeader::name_as_string;
        Printer p(get_max());
        ...
        p(bin_hdr.ext_samp_per_tr_orig,
		  name_as_string(name::EXT_SAMP_PER_TR_ORIG));
        p(bin_hdr.ext_ens_fold, name_as_string(name::EXT_ENS_FOLD));
        p(bin_hdr.endianness, name_as_string(name::ENDIANNESS));
        p(bin_hdr.SEGY_rev_major_ver,
		  name_as_string(name::SEGY_REV_MAJOR_VER));
        p(bin_hdr.SEGY_rev_minor_ver,
		  name_as_string(name::SEGY_REV_MINOR_VER));
        p(bin_hdr.fixed_tr_length, name_as_string(name::FIXED_TR_LENGTH));
        p(bin_hdr.ext_text_headers_num,
		  name_as_string(name::EXT_TEXT_HEADERS_NUM));
        p(bin_hdr.max_num_add_tr_headers,
		  name_as_string(name::MAX_NUM_ADD_TR_HEADERS));
        ...
        p(bin_hdr.byte_off_of_first_tr,
		  name_as_string(name::BYTE_OFF_OF_FIRST_TR));
        p(bin_hdr.num_of_trailer_stanza,
		  name_as_string(name::NUM_OF_TRAILER_STANZA));
    }
